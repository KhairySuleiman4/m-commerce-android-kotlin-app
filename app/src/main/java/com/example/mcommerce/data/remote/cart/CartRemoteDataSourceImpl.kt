...
class CartRemoteDataSourceImpl(private val graphQLService: GraphQLService) : CartRemoteDataSource {
    override fun getCartById(cartId: String): Flow<ApiResult<CartEntity?>> = executeAPI { graphQLService.getCartById(cartId).data?.toModel()?.toEntity() }

    override fun createCart(accessToken: String, email: String): Flow<ApiResult<CartEntity?>> = executeAPI {
    ...
    override fun addItemToCart(
        ...
        quantity: Int,
        itemId: String
    ): Flow<ApiResult<CartEntity?>> = executeAPI { graphQLService.addItemToCart(cartId, quantity, itemId).data?.toModel()?.toEntity() }

    override fun removeItemFromCart(cartId: String, itemId: String): Flow<ApiResult<CartEntity?>> = executeAPI { graphQLService.removeItemFromCart(cartId, itemId).data?.toModel()?.toEntity() }
